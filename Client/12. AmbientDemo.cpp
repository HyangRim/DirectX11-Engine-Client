...
void AmbientDemo::Init()
{
	...
	{
		auto shader = make_shared<Shader>(L"09. Ambient.fx");
		m_shader = shader;
...
void AmbientDemo::Update()
{
	...
	m_shader->GetVector("LightAmbient")->SetFloatVector((float*)&lightAmbient);
	//RENDER->Update();
	//RENDER->Update();
	{
		Vec4 materialAmbient(1.f);
		m_shader->GetVector("MaterialAmbient")->SetFloatVector((float*)&materialAmbient);
		m_obj->Update();
	}
	{
		Vec4 materialAmbient(1.f);
