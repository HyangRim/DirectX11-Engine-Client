...
void DiffuseDemo::Init()
{
	...
	{
		auto shader = make_shared<Shader>(L"10. Diffuse.fx");
		m_shader = shader;
...
void DiffuseDemo::Update()
{
	...
	m_shader->GetVector("LightDiffuse")->SetFloatVector((float*)&lightDiffuse);
	...
	{
		Vec4 material(1.f);
		m_shader->GetVector("MaterialDiffuse")->SetFloatVector((float*)&material);
		m_obj->Update();
	}
	{
		Vec4 material(1.f);
