...
BiancaAnimEState::BiancaAnimEState()
    : AnimationState(AnimationStateType::Skill_3)
{
}

void BiancaAnimEState::Enter(shared_ptr<ModelAnimator> animator)
{
    if (!animator)
        return;
...
void BiancaAnimEState::Update(shared_ptr<ModelAnimator> animator)
{
    if (!animator)
        return;
...
void BiancaAnimEState::HandleSkillInput()
{
    // E 키 해제 감지
...
void BiancaAnimEState::HandleMovementInput()
{
...
void BiancaAnimEState::UpdateCharging()
{
    if (!m_cachedAnimator)
        return;
...
void BiancaAnimEState::UpdateReleasing()
{
    if (!m_cachedAnimator)
        return;
    ...
    {
        m_isComplete = true;
        cout << "비앙카 E 스킬 애니메이션 완료!" << endl;
    }
}

void BiancaAnimEState::UpdateEnding()
{
    if (!m_cachedAnimator)
        return;
    ...
    {
        m_isComplete = true;
        cout << "비앙카 E 스킬 애니메이션 완료!2" << endl;
    }
}

shared_ptr<GameObject> BiancaAnimEState::GetGameObject() const
{
    if (m_cachedAnimator)
    {
...
void BiancaAnimEState::ReleaseSkill()
{
    if (m_isReleasing)
        return;
    ...
    cout << "비앙카 E 스킬 애니메이션 발동! 차징 시간: " << m_chargeTime << "초" << endl;
...
void BiancaAnimEState::TransitionToSkillState(BiancaESkillChargeState newState)
{
...
void BiancaAnimEState::Exit(shared_ptr<ModelAnimator> animator)
{
    if (!animator)
        return;

    cout << "비앙카 E 스킬 애니메이션 상태 종료 "  << endl;
