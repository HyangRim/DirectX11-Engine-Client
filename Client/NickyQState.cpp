...
void NickyQState::Enter(shared_ptr<ModelAnimator> animator)
{
    if (!animator)
        return;
...
void NickyQState::Update(shared_ptr<ModelAnimator> animator)
{
    if (!animator)
        return;
...
void NickyQState::HandleSkillInput()
{
    // Q 키 해제 감지
...
void NickyQState::HandleMovementInput()
{
...
void NickyQState::UpdateReleasing()
{
    if (!m_cachedAnimator)
        return;
...
void NickyQState::SetInitialMovementState(bool wasMoving)
{
...
shared_ptr<GameObject> NickyQState::GetGameObject() const
{
    if (m_cachedAnimator)
    {
...
void NickyQState::TransitionToChargeState(QSkillChargeState newState)
{
...
void NickyQState::PlayAppropriateAnimation()
{
    if (!m_cachedAnimator)
        return;
...
bool NickyQState::IsStartAnimationComplete()
{
    if (!m_cachedAnimator)
        return true;
...
void NickyQState::Exit(shared_ptr<ModelAnimator> animator)
{
    if (!animator)
        return;
...
bool NickyQState::CanTransitionTo(AnimationStateType nextState)
{
    // 스킬이 완료되었을 때만 Wait 상태로 전환 가능
    return (m_isComplete && nextState == AnimationStateType::Wait);
}

bool NickyQState::IsCharging() const
{
    return m_isChargingActive;
}

bool NickyQState::IsComplete() const
{
    return m_isComplete;
}
