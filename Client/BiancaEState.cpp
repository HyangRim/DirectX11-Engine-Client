...
void BiancaEState::Enter(shared_ptr<ModelAnimator> animator)
{
    if (!animator)
        return;
...
void BiancaEState::Update(shared_ptr<ModelAnimator> animator)
{
    if (!animator)
        return;
...
void BiancaEState::HandleSkillInput()
{
    // E 키 해제 감지
...
void BiancaEState::HandleMovementInput()
{
...
void BiancaEState::UpdateReleasing()
{
    if (!m_cachedAnimator)
        return;
...
void BiancaEState::UpdateEnding()
{
    if (!m_cachedAnimator)
        return;
...
shared_ptr<GameObject> BiancaEState::GetGameObject() const
{
    if (m_cachedAnimator)
    {
...
void BiancaEState::TransitionToSkillState(BiancaESkillChargeState newState)
{
...
void BiancaEState::Exit(shared_ptr<ModelAnimator> animator)
{
    if (!animator)
        return;
...
bool BiancaEState::CanTransitionTo(AnimationStateType nextState)
{
    // 스킬이 완료되었을 때만 Wait 상태로 전환 가능
    return m_isComplete && nextState == AnimationStateType::Wait;
}

bool BiancaEState::IsCharging() const
{
    return m_isCharging;
}

bool BiancaEState::IsComplete() const
{
    return m_isComplete;
}
