// NickyBaseAttackState.cpp (수정된 버전)
#include "pch.h"
#include "NickyBaseAttackState.h"
#include "Player.h"
#include "Monster.h"

NickyBaseAttackState::NickyBaseAttackState(shared_ptr<ModelAnimator> modelAnimator, shared_ptr<GameObject> player)
...
void NickyBaseAttackState::UpdateMovementToTarget()
{
    if (!IsInAttackRange())
    {
        // 주기적으로 경로 업데이트
...
void NickyBaseAttackState::UpdateAttackLogic()
{
    // 공격 범위를 벗어났는지 체크
    if (!IsInAttackRange())
    {
...
void NickyBaseAttackState::CheckForContinuousAttack()
{
    // 여기서 연속 공격 조건을 체크
    // 1. 타겟이 여전히 유효한가?
    // 2. 타겟이 공격 범위 내에 있는가?
...
bool NickyBaseAttackState::IsInAttackRange() const
{
...
void NickyBaseAttackState::RotateToTarget()
{
    if (!m_target) return;
...
void NickyBaseAttackState::DealDamage()
{
    if (!m_target) return;
